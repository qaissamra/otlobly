...
| `az_carts.py` | **The hand-off to AZ Studio, both ways** (2026-09-11): ticked orders (or a PO) become one cart for one buying account (`az_carts` table); AZ Studio takes it by its own poll (`/api/worker/az_carts`, worker token) or by a direct push (`az.send_cart`, bridge token), makes one task, and posts every step back to `/api/worker/az_result`; when the buyer types the Amazon order number there, `apply_ordered` writes the purchase order here and flips the customer orders to ORDERED. UI: **Send to AZ Studio** on To order / In cart / a PO row, the AZ Studio chips, the bell group. |
